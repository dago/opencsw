 ...
 */
#define DEFSPATH	"PATH=/usr/bin:/usr/sbin"			/* */
#define DEFPATH	"PATH=$HOME/bin:/usr/bin"		/* */
...
#define ETCRC	"/etc/opt/csw/procmailrc"	/* optional global procmailrc startup
					   file (will only be read if procmail
	is started with no rcfile on the command line). */

#define ETCRCS	"/etc/opt/csw/procmailrcs/"	/* optional trusted path prefix for
...
#define MMAP_DIR	"/var/opt/csw/procmail/"		     /* where to put */
#endif								/* the files */
